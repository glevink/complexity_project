...
df_text= pd.read_csv('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/applied_models/applied_model_leader_batch_1-3.csv')
...
#write df to csv
jobtext_outf= open('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/word_clouds_jobtext.csv', 'w+', encoding= 'utf-8')
...
plt.text(600, 1150, "Mean   : " + str(round(np.mean(jobtext_lens_words))))
plt.text(600, 1120, "Median : " + str(round(np.median(jobtext_lens_words))))
plt.text(600, 1090, "Stdev   : " + str(round(np.std(jobtext_lens_words))))
plt.text(600, 1060, "1%ile    : " + str(round(np.percentile(jobtext_lens_words, 1))))
plt.text(600, 1030, "99%ile  : " + str(round(np.percentile(jobtext_lens_words, 99))))
plt.text(600, 1000, "Max : " + str(round(np.max(jobtext_lens_words))))
plt.gca().set(xlim= (0, 800), ylabel= 'Number of Job Postings', xlabel= 'Job Posting Word Count')
plt.tick_params(size= 16)
plt.xticks(np.linspace(0, 800, 11))
plt.title('Distribution of Job Posting Word Counts', fontdict= dict(size= 14))
plt.show()    
plt.savefig('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/distr_jobtext_words.png')    

#plot 2- number of tokens
plt.figure(figsize= (10,7), dpi= 160)
plt.hist(jobtext_lens_tokens, bins= 100, color= 'purple')
plt.text(600, 1150, "Mean   : " + str(round(np.mean(jobtext_lens_tokens))))
plt.text(600, 1120, "Median : " + str(round(np.median(jobtext_lens_tokens))))
plt.text(600, 1090, "Stdev   : " + str(round(np.std(jobtext_lens_tokens))))
plt.text(600, 1060, "1%ile    : " + str(round(np.percentile(jobtext_lens_tokens, 1))))
plt.text(600, 1030, "99%ile  : " + str(round(np.percentile(jobtext_lens_tokens, 99))))
plt.text(600, 1000, "Max : " + str(round(np.max(jobtext_lens_tokens))))
plt.gca().set(xlim= (0, 800), ylabel= 'Number of Job Postings', xlabel= 'Job Posting Token Count')
plt.tick_params(size= 16)
plt.xticks(np.linspace(0, 800, 11))
plt.title('Distribution of Job Posting Token Counts', fontdict= dict(size= 14))
plt.show()    
plt.savefig('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/distr_jobtext_tokens.png')    
...
wcloud.to_image()
wcloud.to_file('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/wcloud_top10predict.png') 
     
    
### Actual Top 10%
criterion = 'leaderComplexityOCC'
cutoff = np.percentile(df_text[criterion], 90)
...
#Visualize the word cloud
wcloud.to_image()
wcloud.to_file('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/wcloud_top10act.png') 
...
#Visualize the word cloud
wcloud.to_image()
wcloud.to_file('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/wcloud_low10predict.png') 
...
#Visualize the word cloud
wcloud.to_image()
wcloud.to_file('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/wcloud_low10act.png') 
...
top_m_low_predict_mat = [max(0,x) for x in top_m_low_predict_mat[0]]                            
...
plt.subplot(title='20 most distinct words: top vs. bottom predicted score decile')
sns.set_context("notebook", font_scale=1.5, rc={"lines.linewidth": 2.5})
sns.barplot(x_pos, counts, palette='husl')
plt.xticks(x_pos, words, rotation=20) 
plt.xlabel('words')
plt.ylabel('counts')
plt.savefig('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/top_v_low_pred_freq.png')
plt.show()

    ### Difference count arrays top vs. bottom ACTUAL and plot ###

top_m_low_act_mat = top10act_mat - low10act_mat
top_m_low_act_mat = [max(0,x) for x in top_m_low_act_mat[0]]                            
...
plt.figure(2, figsize=(15, 15/1.6180))
plt.subplot(title='20 most distinct words: top vs. bottom actual score decile')
sns.set_context("notebook", font_scale=1.5, rc={"lines.linewidth": 2.5})
sns.barplot(x_pos, counts, palette='husl')
plt.xticks(x_pos, words, rotation=20) 
plt.xlabel('words')
plt.ylabel('counts')
plt.savefig('/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/top_v_low_act_freq.png')
plt.show()
...
#visualize with FreqDistVisualizer specifying for top 10 words
#default is 50 words
visualizer= FreqDistVisualizer(features= features, n= 20, orient= "v")
visualizer.fit(jobtext_vector)
visualizer.poof()
visualizer.show(outpath='/Users/gschubert/Dropbox/PhD Course Notes/6.862 Machine Learning/complexity_project/Project proposal/freq_top20_tokens_A.png')





#Top 10 most frequent words - using sklearn's Count_Vecotrizer function & yellowbrick's FreqDistVisualizer function 
#Helper function
...
# Features
jobtext_corpus= df_text['jobtext_tokens'].map(lambda x: str(x))
jobtext_vector= count_vectorizer.fit_transform(jobtext_corpus)
